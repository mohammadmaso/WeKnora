export default {
  ...
  integrations: {
    ...
    claw: {
      ...
      subtitle: '通过 Mohaymen EKMS REST API 导入文档并执行混合检索（向量 + 关键词）。适用于上传文件/URL/Markdown 到知识库、跨库检索与浏览知识内容。',
      ...
      installCtaHint: '安装 Mohaymen EKMS Skill · 将在新标签页打开',
    ...
    chrome: {
      title: '知识管理助手',
      subtitle: '配合 Mohaymen EKMS 自建服务使用：在任意网页侧边栏提问、剪藏内容、Markdown 速记，将浏览中的知识沉淀到你的知识库。',
      ...
      steps: {
        ...
        port: {
          title: '桌面版配置固定端口（推荐）',
          desc: '使用 Mohaymen EKMS 桌面版时，在 API 信息中设置固定端口（如 37841），避免每次启动后地址变化导致插件断连。'
        },
        api: {
          title: '获取 API 凭证',
          desc: '在「设置 → API 信息」中复制 API Key 与 API 地址。'
    ...
    api: {
      ...
      signedFlowDetail: 'HMAC 密钥仅保存在 Mohaymen EKMS 与你的可信后端，切勿放入请求或下发给浏览器。请求头里传的是用该密钥签出的 JWT 字符串（每次签发或过期后会变），不是密钥本身。JWT 须包含 sub（外部用户 ID）、tenant_id、aud=weknora、exp（有效期不超过 24 小时）。',
      ...
      hmacSecret: 'HMAC 密钥',
      hmacSecretDesc: '与 Mohaymen EKMS 保存的同一份密钥；仅用于后端签发 JWT，不要作为请求头发送。',
  ...
  embedPublish: {
    ...
    webhookUrlDesc: '可选。访客发消息与助手回复完成时，Mohaymen EKMS 会向该 HTTPS 地址 POST 事件（message_sent / message_received）。',
  ...
  createChat: {
    title: 'Hi，我是 Mohaymen EKMS，让你的知识触手可及',
    newSessionTitle: '新会话',
    messages: {
      createFailed: '创建会话失败',
  ...
  auth: {
    ...
    firstTime: '首次使用 Mohaymen EKMS？',
    registerSuccess: '注册成功，请登录',
    registerFailed: '注册失败',
    subtitle: 'RAG 问答、ReAct 智能体与 Wiki 知识库，大模型驱动的企业级知识框架',
    registerSubtitle: '创建账户并开始使用 Mohaymen EKMS',
  ...
  settings: {
    ...
    sandbox: {
      ...
      backendDescriptions: {
        ...
        docker: '每次在当前 Mohaymen EKMS 主机启动一个短生命周期容器执行脚本',
        local: '直接在 Mohaymen EKMS 服务进程所在环境执行脚本',
      },
      addConfig: '添加沙箱后端',
      ...
      weknoraTemplateTitle: 'Mohaymen EKMS 标准模板',
      weknoraDockerImage: 'Mohaymen EKMS 标准镜像',
      weknoraDockerImageHint: '每次执行都会基于该镜像创建一个隔离的短生命周期容器，不保留会话沙箱。',
      localRuntimeWarning: '本地进程会直接在 Mohaymen EKMS 服务器上执行技能脚本，不具备容器或 MicroVM 隔离。仅建议在可信的开发空间使用。',
      weknoraTemplateOverview: 'Mohaymen EKMS 负责提供标准运行环境。连接集群后会自动发现模板，缺少标准模板时自动创建。',
      weknoraTemplateDescription: '包含 Mohaymen EKMS 技能所需的 Python、Node.js、CLI 工具、工作目录和非 root 执行用户。',
      recommendedTag: '推荐',
      ...
      localRuntimeSummary: 'Mohaymen EKMS 服务器本地进程',
      templateApplied: '已使用',
      refreshTemplates: '刷新模板',
      templateSelectHelp: '模板由当前集群实时返回，保存时仅记录模板 ID，无需手工复制。',
      templateSelectPlaceholder: '先连接集群并加载模板',
      templateLoadHint: '填写集群连接后刷新模板；若缺少 Mohaymen EKMS 标准模板，系统会自动创建。',
      templateLoadFailed: '模板列表加载失败',
      standardTemplateProvisioning: '已开始在集群中创建 Mohaymen EKMS 标准模板，请稍后刷新查看状态',
      templateNotReady: '所选模板尚未构建完成，请刷新并等待状态就绪',
      connectionPassed: '连接验证通过，以下模板均从当前集群实时加载。',
      connectionPassedTitle: '集群连接成功',
      templateStepHint: '集群缺少标准模板时，Mohaymen EKMS 会自动创建；模板就绪后才能继续。',
      loadingTemplates: '正在从集群加载模板…',
      templateBuildingHint: '标准模板正在自动构建，列表会自动刷新。',
      templateUntaggedHint: '构建已完成，但没有构建带 default 标签，创建沙箱时无法解析。请在 E2B 删除该模板，刷新后 Mohaymen EKMS 会重新构建。',
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        title: '创建你的知识库',
        desc: '知识库是一切的起点：上传文档、网页或 FAQ，Mohaymen EKMS 会自动解析并建立索引。点击这里进入知识库。'
      },
      welcome: {
        title: '欢迎使用 Mohaymen EKMS',
